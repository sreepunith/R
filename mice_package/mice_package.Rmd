...
str(dt) # Inspect the data structure

# Remove unnecessary columns
dt[, c("PassengerId", "Name", "Cabin", "Ticket")] <- NULL
summary(dt) # Summary the data
...
```{r}
marginplot(dt[, c("Fare", "Age")], col = c("navyblue", "red"), numbers = TRUE, pch = 19)
```
...
stripplot(Age~.imp, data = dt_imp_1, jit = TRUE, fac = 0.8, col = col, xlab = "Imputation Number")

# Fare
col <- rep(c("blue", "red")[1+as.numeric(is.na(imp$data$Fare))], 6)
stripplot(Fare~.imp, data = dt_imp_1, jit = TRUE, fac = 0.8, col = col, xlab = "Imputation Number")
```
...
```{r}
fit <- with(imp, glm(Survived ~ Sex + Age + Fare + Pclass + SibSp + Parch + Embarked, 
                     family = "binomial"))
```
...
Buuren, S. & Groothuis-Oudshoorn, K. (2011, December). MICE: Multivariate Imputation by Chained Equations in R. Journal of Statistical Software, 45(3), 1–67. Retrieved from http://doc.utwente.nl/78938/
