...
restricted <- mice(train, m = 5000, post = post, seed = 345, method = 'norm.predict')
...
# STATISTICS TEST
################################################################################
flds <- createFolds(train$Survived, k = 10, list = TRUE, returnTrain = FALSE)
names(flds)[1] <- "train"
...
# Model tuning
grid <-  expand.grid(mtry = seq(1, 100, 1))
