...
p1 <- ggplot() +
  ...
  geom_text(data = data.frame(grad_result_1), aes(x = x, y = y, label = id), vjust=-1, colour = "blue") +
...
p2 <- ggplot() +
  ...
  geom_text(data = data.frame(grad_result_3), aes(x = x, y = y, label = id), vjust=-1, colour = "blue") +
...
p3 <- ggplot() +
  ...
  geom_text(data = data.frame(grad_result_5), aes(x = x, y = y, label = id), vjust=-1, colour = "blue") +
  ggtitle(expression(paste(eta, " = 0.5")))

p4 <- ggplot() +
  geom_line(data = df, mapping = aes(x = x, y = y), linetype="dashed", colour = "red") +
  geom_path(data = data.frame(grad_result_7), aes(x = x, y = y), colour = "blue") +
  geom_point(data = data.frame(grad_result_7), aes(x = x, y = y)) +
  geom_text(data = data.frame(grad_result_7), aes(x = x, y = y, label = id), vjust=-1, colour = "blue") +
  ggtitle(expression(paste(eta, " = 0.7")))

# Use extra function for multiple plot
multiplot(p1, p3, p2, p4, cols=2)
dev.copy(png,filename="figs/gradient_desc_quad_func.png")
dev.off ()
