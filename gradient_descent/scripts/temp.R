...
newton_opt <- function(x_init){
  ...
  for(indx in 2:m) {
    #compute new x
    x <- x - solve(second_order_derivative(x)) * first_order_derivative(x) 
    #compute new value of y by new x
    y <- poly_func(x)
    # update the result
    result[indx,] <- c(indx, x, y)
...
draw.curve<-function(indx){
  p1 <- ggplot(data = df, mapping = aes(x = x, y = y)) +
    ...
    geom_path(data = data.frame(newton_result[1:indx,]), aes(x = x, y = y), colour = "green") +
    geom_point(data = data.frame(newton_result[1:indx,]), aes(x = x, y = y)) +
...
trace.animate <- function() {
  lapply(seq(1,4,1), function(i) {
    draw.curve(i)
  })
}

# save all iterations into one GIF
# must install imagemagick to run
# brew install imagegick
saveGIF(trace.animate(), interval = .8, movie.name="four_plots.gif")
