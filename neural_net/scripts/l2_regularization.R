...
# multi-class classification task on the Iris flower dataset. The purpose is to
# demonstrate the benefits of regularisation, specifically l-2 regularisation.
...
# + Regularaisation: L2 regularisation & No regularisation
################################################################################
# LOAD LIBRARY
################################################################################
library(mlbench)
...
summary(dt) 
################################################################################
# BACKPROPAGATION + L2 REGULARISATION
################################################################################
set.seed(65432)
...
################################################################################
# BACKPROPAGATION NO REGULARISATION
################################################################################
set.seed(65432)

no_reg_softmax_training <- function(x_idx, #column indexes of input vectors 
                                y_idx, #column indexes of output vectors
                                data = NULL,
                                training_percent = 0.8, #portion of data for training
                                learning_rate = 1e-3, #learning rate
  ...
  for (j in 1:iters) {
    ...
    log_prob <- -log(matching_probs)
    cross_entropy_loss  <- sum(log_prob)/training_size
    loss <- c(loss, cross_entropy_loss)
...
pred1 <- l2_softmax_training(x_idx = x_idx, y_idx = y_idx, data = data,
                            training_percent = training_percent,
                            learning_rate = learning_rate,
                            regularization_rate = regularization_rate,
                            iters = iters)
pred2 <- no_reg_softmax_training(x_idx = x_idx, y_idx = y_idx, data = data,
                               training_percent = training_percent,
                               learning_rate = learning_rate,
                               iters = iters)

# Collect data for report
report1 <- data.frame(training_accuracy = pred1$training_accuracy,
                     testing_accuracy = pred1$testing_accuracy,
                     loss = pred1$loss,
                     epoch = 1:iters)
report2 <- data.frame(training_accuracy = pred2$training_accuracy,
                      testing_accuracy = pred2$testing_accuracy,
                      loss = pred2$loss,
                      epoch = 1:iters)
report1 <- report1[which((report1$epoch %% 500) == 0),] #keep less epoches
report2 <- report2[which((report2$epoch %% 500) == 0),] #keep less epoches

# Plot
ggplot() +
  geom_path(data = report1, aes(x = epoch, y = loss, colour = "L2 regularisation")) +
  geom_point(data = report1, aes(x = epoch, y = loss, colour = "L2 regularisation"), shape = 0) + 
  geom_path(data = report2, aes(x = epoch, y = loss, colour = "No regularisation")) +
  geom_point(data = report2, aes(x = epoch, y = loss, colour = "No regularisation"), shape = 21) + 
  xlab("Epoch") + ylab("") +
  ggtitle("Cost function of \nL2 regularisation vs No regularisation") + 
  theme(legend.title = element_blank(), legend.position = c(.9, .93), 
        legend.key.size = unit(1, "cm"), legend.text = element_text(size = 12))

ggsave("figs/cost_l2_vs_no_reg.png")

ggplot() +
  geom_path(data = report1, aes(x = epoch, y = training_accuracy, colour = "L2 regularisation")) +
  geom_point(data = report1, aes(x = epoch, y = training_accuracy, colour = "L2 regularisation"), shape = 0) + 
  geom_path(data = report2, aes(x = epoch, y = training_accuracy, colour = "No regularisation")) +
  geom_point(data = report2, aes(x = epoch, y = training_accuracy, colour = "No regularisation"), shape = 21) + 
  xlab("Epoch") + ylab("") +
  ggtitle("Training accuracy of \nL2 regularisation vs No regularisation") + 
  theme(legend.title = element_blank(), legend.position = c(.9, .93), 
        legend.key.size = unit(1, "cm"), legend.text = element_text(size = 12))

ggsave("figs/training_accuracy_l2_vs_no_reg.png")

ggplot() +
  geom_path(data = report1, aes(x = epoch, y = testing_accuracy, colour = "L2 regularisation")) +
  geom_point(data = report1, aes(x = epoch, y = testing_accuracy, colour = "L2 regularisation"), shape = 0) + 
  geom_path(data = report2, aes(x = epoch, y = testing_accuracy, colour = "No regularisation")) +
  geom_point(data = report2, aes(x = epoch, y = testing_accuracy, colour = "No regularisation"), shape = 21) + 
  xlab("Epoch") + ylab("") +
  ggtitle("Testing accuracy of \nL2 regularisation vs No regularisation") + 
  theme(legend.title = element_blank(), legend.position = c(.9, .93), 
        legend.key.size = unit(1, "cm"), legend.text = element_text(size = 12))

ggsave("figs/testing_accuracy_l2_vs_no_reg.png")